...
class CycleGAN:
  def __init__(self,
    ...
    self.use_lsgan = use_lsgan #lsgan为GAN的变种
    ...
    self.is_training = tf.palceholder_with_default(True,shape=[],name='is_training')
 ...
 def optimize(self, G_loss, D_Y_loss, F_loss, D_X_loss):
    def make_optimizer(loss, variables, name='Adam'):
      ...
      learning_rate = (
          tf.where(
                  tf.greater_equal(global_step, start_decay_step), #比较运算符，返回x=y的真值
                  tf.train.polynomial_decay(starter_learning_rate, global_step-start_decay_step,
                                            decay_steps, end_learning_rate,
                                            power=1.0), #实现多项式衰减学习率
                  starter_learning_rate
          ) #tf.where()根据条件返回某一value
    ...
    with tf.control_dependencies([G_optimizer, D_Y_optimizer, F_optimizer, D_X_optimizer]): #确保四中function按顺序执行
      return tf.no_op(name='optimizers') #tf.no_op()什么也不做
