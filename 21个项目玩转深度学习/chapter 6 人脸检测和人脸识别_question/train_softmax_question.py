...
def parse_arguments(argv):
    ...
    parser.add_argument('--logs_base_dir', type=str,
                        help='Directory where to write event logs.', default='~/logs/facenet') #添加命令行
...
def save_variables_and_metagraph(sess,saver,summary_writer,model_dir,model_name,step):
    ...
    print('Savingvariables')
    start_time = time.time()
    checkpoint_path = os.path.join(model_dir,'model-%s.ckpt' % model_name)
    saver.save(sess,checkpoint_path,global_step=step,write_meta_graph=False) #保存checkpoint
    ...
    if not os.path.exists(metagraph_filename):
        ...
        start_time = time.time()
        saver.export_meta_graph(metagraph_filename) #exprot_meta_graph()
        save_time_metagraph = time.time() - start_time
        print('Metagraph saved in %.2f seconds' % save_time_metagraph)
    summary = tf.Summary()
    summary.value.add(ta='time/save_variables',simple_value=save_time_variables)
    summary.value.add(tag='time/save_metagraph', simple_value=save_time_metagraph)
    summary_writer.add_summary(summary, step) #将数据添加到tensorboard
