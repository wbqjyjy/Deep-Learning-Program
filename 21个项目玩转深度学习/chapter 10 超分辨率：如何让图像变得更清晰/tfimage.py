...
def create_op(func,**placeholders): #输入的是tensor，返回的是array
    op = func(**placeholders) #这里placeholders是一个字典，是位置参数

    def f(**kwargs):
        feed_dict = {}
        for argname,argvalue in kwargs.items():
            placeholder = placeholders[argname] #create_op关键字参数的值赋给placeholder
            feed_dict[placeholder] = argvalue #该参数对应的value
        return tf.get_default_session().run(op,feed_dict = feed_dict) #返回的是array
...
downscale = create_op(
    ...
    ) #create_op()中的f()并没有赋值，因此，feed_dict无；

upscale = create_op(func = tf.image.resize_images,
                    images = tf.placeholder(tf.float32,[None,None,None]),
                    size = tf.placeholder(tf.int32,[2]),
...
def save(image,path,replace=False): #将Image存入Path中
    _,ext = os.path.splitext(path.lower()) #文件格式
    image = to_uint8(image=image)
    if ext == ".jpg": #将image编码为特定格式
        encoded = encode_jpeg(image=image)
    elif ext == ".png":
        encoded = encode_png(image=image)
    else:
        raise Exception("invalide image suffix")

    dirname = os.path.dirname(path) #返回Path中的dir
    if dirname != "" and not os.path.exists(dirname): #如果该dir在电脑中不存在，则创建
        os.makedirs(dirname)

    if os.path.exists(path): #如果Path已经存在，要么删除，要么引发错误，申明该文件已存在
        if replace:
            os.remove(path)
        else:
            raise Exception("file already exists at" + path)

    with open(path,"wb") as f: #将Image写入path中
        f.write(encoded)
